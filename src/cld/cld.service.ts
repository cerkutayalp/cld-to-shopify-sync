...
axiosRetry(axios, {
  ...
  retryDelay: (retryCount, error) => {
    console.log(`🔁 Retry attempt ${retryCount} due to ${error.code || error.message}`);
...
export class CldService {
  ...
  this.isTestMode = this.configService.get<string>("SHOPIFY_TEST_MODE") === 'true';
...
private async getCldProductsPaginated(url: string, payload: PaginationPayload): Promise<any> {
...
async sendAllProductsToShopify() {
  ...
  for await (const products of this.getStockList()) {
    for (const cldProduct of products) {
      ...
      if (response) {
        sentCount++;
        console.log(`✅ Sent product #${sentCount}:`, response.product?.id || response);
      }
    }
  }
  ...
  async sendFirstFiveProductsToShopify() {
    ...
    for await (const products of this.getStockList()) {
      for (const cldProduct of products) {
        ...
        const shopifyProduct = this.mapCldToShopifyProduct(cldProduct);
        const response = await this.sendProductToShopify(shopifyProduct);
        console.log(`Sent product ${sentCount + 1}:`, response.product?.id || response);
        sentCount++;
        if (sentCount >= 5) return;
      }
    }
  ...
  for await (const page of this.getStockList()) {
    ...
    if (product) {
      ...
      console.log(`✅ Sent product ${cldIdentifier} to Shopify:`, shopifyResponse.product?.id || shopifyResponse);
      return shopifyResponse;
    }
  }

  throw new Error(`❌ Product with identifier ${cldIdentifier} not found in stock list.`);
}
